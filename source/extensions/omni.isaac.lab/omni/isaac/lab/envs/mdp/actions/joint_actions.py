...
class SyncedJointPositionAction(JointPositionAction):
    """Joint action term that applies the processed actions to the articulation's joints as position commands."""

    cfg: actions_cfg.SyncedJointPositionActionCfg
    """The configuration of the action term."""
    ...
    def __init__(self, cfg: actions_cfg.SyncedJointPositionActionCfg, env: ManagerBasedEnv):
        ActionTerm.__init__(self, cfg, env)
        preprocess_joint_ids, _ = self._asset.find_joints(self.cfg.joint_names)
        self._action_dim = len(preprocess_joint_ids)
        super().__init__(cfg, env)
        self._action_dim = 1
        # use default joint positions as offset
        if cfg.use_default_offset:
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        # store the raw actions
        # self._raw_actions[:, :] = actions
        # print("finger self._raw_actions:", self._raw_actions)
        self._raw_actions[:, 0] = actions[:, 0] 
        self._raw_actions[:, 1] = actions[:, 0]
        # apply the affine transformations
        self._processed_actions = self._raw_actions * self._scale + self._offset
        # print("finger self._processed_actions:", self._processed_actions)
