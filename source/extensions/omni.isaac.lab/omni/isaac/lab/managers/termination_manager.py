...
class TerminationManager(ManagerBase):
    ...
    * **Terminated**: This signal is set to true if the environment has reached a terminal state defined by the
    ...
    def compute_unexpected_terminal(self) -> torch.Tensor:
        """Computes the termination signal for the 'state_out_of_bounds' term only.

        This function calls the 'state_out_of_bounds' termination term managed by the class and returns its value as the termination signal.

        Returns:
            The termination signal of "state_out_of_bounds" with shape (num_envs,).
        """
        self._terminated_buf[:] = False

        # iterate over all the termination terms to find 'state_out_of_bounds'
        for name, term_cfg in zip(self._term_names, self._term_cfgs):
            if name == "state_out_of_bounds":
                # Compute the value for 'state_out_of_bounds'
                value = term_cfg.func(self._env, **term_cfg.params)
                self._terminated_buf |= value
