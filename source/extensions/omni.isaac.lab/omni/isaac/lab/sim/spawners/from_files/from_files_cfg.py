...
@configclass
class RandomUsdFileCfg(RigidObjectSpawnerCfg):
    func: Callable = from_files.spawn_from_random_usd

    scale_mode: str = MISSING
    """uniform, discret"""
    
    scales: list[tuple[float, float, float]] | tuple[tuple[float, float], tuple[float, float], tuple[float, float]] | None = None
    """List of scales to apply to the asset. Defaults to None, in which case the scale is not modified. Defaults to (1.0, 1.0, 1.0)."""
    ...
    visual_material_paths: list[str] | None =  None
...
@configclass
class OmniGibsonUsdFileCfg(UsdFileCfg):

    func: Callable = from_files.spawn_from_omnigibson_usd

    usd_path: str = MISSING
    
    @configclass
    class OmniGibsonAttrCfg:
        """Attributes to apply to the OmniGibson object."""
        ...
        fixed_base: bool = MISSING
        """Whether the object is fixed to the base."""
    
    omnigibson_attr: OmniGibsonAttrCfg = MISSING
